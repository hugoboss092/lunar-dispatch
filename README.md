...
3. Выберите тактику маршрута, проверьте расход батареи, время, риск и выплату, затем запустите доставку.
...
- Тактика меняет итоговый маршрут: экономичная бережёт заряд, быстрая сокращает время и даёт надбавку, осторожная снижает риск ценой части награды.
...
- три тактики маршрута с разным расходом, временем, риском и выплатой;
- журнал событий и сохранение после каждого изменения;
- 19 автоматических тестов игровой логики и базового состава интерфейса;
- desktop и mobile layouts, keyboard focus и режим reduced motion.

## Технологии

React 19, TypeScript, Vite, Vitest, Lucide Icons и CSS без UI-фреймворка. Маршруты, маркеры, зоны и состояния карты сделаны собственными HTML/CSS/SVG-средствами. Для рельефа используется локальная WebP-текстура лунной поверхности, поэтому игра не зависит от внешнего CDN. Google Fonts используются только для типографики; при отсутствии сети применяются системные fallback-шрифты.

## Использование AI

AI использовался как помощник при проектировании игровой механики, создании визуального направления, написании кода и тест-кейсов, а также для генерации исходной лунной текстуры. Текстура оптимизирована и хранится локально в проекте. Готовые AI API в работающем приложении не используются: игра полностью работает локально и не требует ключей или платных сервисов.
